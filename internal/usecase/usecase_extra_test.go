...
func TestClearCurrentOperationJournal(t *testing.T) {
	...
	svc := NewProfileService(&fakeProfileRepo{}, &fakeStateRepo{}, nil, nil, nil)
	svc.SetOperationJournal(journal)
...
func TestBackupListAndHelpers(t *testing.T) {
	...
	svc := NewProfileService(&fakeProfileRepo{}, state, nil, nil, nil)
	...
	state := &fakeStateRepo{
		state: domainprofile.State{
			...
			CodexProfiles: map[string]domainprofile.CodexProfileBinding{
				"relay-a": {Status: domainprofile.BindingStatusApplied, ConfigPath: "/tmp/codex/config.toml"},
			},
		},
	}
...
func TestRestoreAgentSnapshot(t *testing.T) {
	...
	svc := NewProfileService(&fakeProfileRepo{}, &fakeStateRepo{}, codex, claude, gemini)
...
func TestDoctorHelpersAndOperationHelpers(t *testing.T) {
	...
	checkCodexBindings(report, domainprofile.State{
		...
		CodexProfiles: map[string]domainprofile.CodexProfileBinding{
			"missing": {Status: domainprofile.BindingStatus("broken")},
		},
	}, map[string]struct{}{})
...
func TestMutationGuardErrorBranches(t *testing.T) {
	guardSvc := NewProfileService(&errorProfileRepo{getErr: errors.New("repo get failed")}, &fakeStateRepo{}, nil, nil, nil)
	guard := newMutationGuard(guardSvc)
	if err := guard.CaptureProfile("relay-a"); err == nil {
		t.Fatal("CaptureProfile() unexpectedly succeeded")
	}

	stateSvc := NewProfileService(&fakeProfileRepo{}, &errorStateRepo{loadErr: errors.New("state load failed")}, nil, nil, nil)
	guard = newMutationGuard(stateSvc)
	if err := guard.CaptureState(); err == nil {
		t.Fatal("CaptureState() unexpectedly succeeded")
	}

	guard = newMutationGuard(NewProfileService(&fakeProfileRepo{}, &fakeStateRepo{}, nil, nil, nil))
	if err := guard.CaptureAgents(domainprofile.Agent("bad")); err != nil {
		t.Fatalf("CaptureAgents(invalid) error = %v, want nil skip", err)
	}

	repoSvc := NewProfileService(&errorProfileRepo{deleteErr: errors.New("delete failed")}, &fakeStateRepo{}, nil, nil, nil)
...
func TestRemoveCodexProfileArtifactsBranches(t *testing.T) {
	...
	if _, ok := state.state.CodexProfiles["relay-a"]; ok {
		t.Fatalf("CodexProfiles still contains relay-a: %+v", state.state.CodexProfiles)
	}
}
...
func TestSnapshotAndRestoreAgentConfigBranches(t *testing.T) {
	...
	codex := &fakeCodexSyncer{codexBase}
	svc = NewProfileService(&fakeProfileRepo{}, &fakeStateRepo{}, codex, nil, nil)
	...
	svc = NewProfileService(&fakeProfileRepo{}, &fakeStateRepo{}, nil, claude, nil)
...
func TestRollbackBindingChangesBranches(t *testing.T) {
	...
	saveErrSvc := NewProfileService(repo, &errorStateRepo{saveErr: errors.New("save failed")}, codex, claude, nil)
...
func TestRollbackBindingChangesRestoresOriginalAgentConfig(t *testing.T) {
	...
	t.Run("bind restores prior unbound config", func(t *testing.T) {
		...
		claude := &fakeClaudeSyncer{claudeBase}
		svc := NewProfileService(repo, state, nil, claude, nil)
...
func TestRemoveRestoreAndDoctorAdditionalErrorBranches(t *testing.T) {
	...
	if _, err := svc.Remove("relay-a"); err == nil || !strings.Contains(err.Error(), "remove profile failed") {
		t.Fatalf("Remove() err=%v, want codex cleanup failure", err)
	}
	if _, err := repo.Get("relay-a"); err != nil {
		t.Fatalf("profile was not rolled back after Remove() failure: %v", err)
	}
	if _, ok := state.state.CodexProfiles["relay-a"]; !ok {
		t.Fatalf("CodexProfiles = %+v, want relay-a restored after rollback", state.state.CodexProfiles)
	}

	svc = NewProfileService(repo, &fakeStateRepo{}, codex, nil, nil)
	...
	svc = NewProfileService(repo, restoreState, codex, nil, nil)
...
func TestApplyRelayBindingChangesAndRestoreBindingChangeAdditionalBranches(t *testing.T) {
	...
	loadErrSvc := NewProfileService(repo, &errorStateRepo{loadErr: errors.New("state load failed")}, nil, nil, gemini)
	...
	claude := &fakeClaudeSyncer{claudeBase}
	gemini = &fakeGeminiSyncer{newFakeAgentSyncer("/tmp/gemini/.env")}
	restoreSvc := NewProfileService(repo, state, codex, claude, gemini)
	...
	noClaudeSvc := NewProfileService(repo, state, codex, nil, gemini)
...
func TestUsecaseErrorEdgeMessagesAndStateHelpers(t *testing.T) {
	...
	if got := codexProfileBinding(domainprofile.State{}, "relay-a"); got != (domainprofile.CodexProfileBinding{}) {
		t.Fatalf("codexProfileBinding(empty state) = %+v, want zero", got)
	}
	...
	svc = NewProfileService(&fakeProfileRepo{}, &fakeStateRepo{state: plainState}, nil, nil, nil)
...
func TestRestoreAndDoctorAdditionalHappyAndErrorBranches(t *testing.T) {
	...
	svc := NewProfileService(&fakeProfileRepo{}, state, nil, claude, gemini)
	...
	journalSvc := NewProfileService(&fakeProfileRepo{}, state, nil, claude, gemini)
	...
	if _, err := NewProfileService(&fakeProfileRepo{}, &errorStateRepo{loadErr: errors.New("load failed")}, nil, nil, nil).BackupList(domainprofile.AgentCodex); err == nil || !strings.Contains(err.Error(), "load failed") {
		t.Fatalf("BackupList(load failure) err=%v, want load failure", err)
	}

	journal := &fakeJournal{}
	doctorSvc := NewProfileService(&fakeProfileRepo{}, &fakeStateRepo{}, nil, nil, nil)
...
func TestAddRemoveRestoreAdditionalFailureBranches(t *testing.T) {
	...
	svc := NewProfileService(repo, stateErr, nil, nil, nil)
	if _, err := svc.Add("relay-a", AddProfileInput{BaseURL: "https://relay.example/v1", APIKey: "sk-a"}); err == nil || !strings.Contains(err.Error(), "save failed") {
	...
	svc = NewProfileService(&fakeProfileRepo{}, restoreState, codex, nil, nil)
...
func TestProfileMutationAdditionalValidationAndFailureBranches(t *testing.T) {
	...
	if _, err := svc.Add("bad name", AddProfileInput{BaseURL: "https://relay.example/v1", APIKey: "sk-a"}); err == nil {
		t.Fatal("Add(invalid name) unexpectedly succeeded")
	}
	if _, err := svc.Add("relay-a", AddProfileInput{BaseURL: "https://relay.example/v1", APIKey: "sk-a"}); err == nil {
		t.Fatal("Add(duplicate) unexpectedly succeeded")
	}

	errSvc := NewProfileService(&errorProfileRepo{getErr: errors.New("repo failed")}, &fakeStateRepo{}, nil, nil, nil)
	if _, err := errSvc.Add("relay-b", AddProfileInput{BaseURL: "https://relay.example/v1", APIKey: "sk-a"}); err == nil || !strings.Contains(err.Error(), "repo failed") {
...
func TestBindingsOperationAdditionalFailureBranches(t *testing.T) {
	...
	svc = NewProfileService(repo, saveErrState, nil, claude, nil)
...
func TestMutationEntrypointsPropagateLockError(t *testing.T) {
	...
	_, err := svc.Add("relay-b", AddProfileInput{BaseURL: "https://relay.example/v1", APIKey: "sk-b"})
	check("Add", err)
...
func TestMutationGuardAndRestoreAdditionalBranches(t *testing.T) {
	...
	claude = &fakeClaudeSyncer{claudeBase}
	svc = NewProfileService(&fakeProfileRepo{}, state, nil, claude, nil)
...
func TestMutationGuardAdditionalBranches(t *testing.T) {
	...
	guard = newMutationGuard(NewProfileService(repo, &fakeStateRepo{}, nil, nil, nil))
	if err := guard.CaptureProfile("relay-a"); err != nil {
	...
	stateGuard := newMutationGuard(NewProfileService(&fakeProfileRepo{}, &fakeStateRepo{}, nil, nil, nil))
	...
	codexBase := newFakeAgentSyncer("/tmp/codex/config.toml")
	codexBase.snapshotContent = []byte("before")
	svc := NewProfileService(&fakeProfileRepo{}, &fakeStateRepo{}, &fakeCodexSyncer{codexBase}, nil, nil)
	guard = newMutationGuard(svc)
	...
	guard = newMutationGuard(NewProfileService(notFoundRepo, &fakeStateRepo{}, nil, nil, nil))
	...
	nilSnapshotSvc := NewProfileService(&fakeProfileRepo{}, &fakeStateRepo{}, nilSnapshotCodex, nil, nil)
	nilSnapshotCodex.fakeAgentSyncer.snapshotContent = nil
...
func TestBindingsEntrypointsAdditionalFailureBranches(t *testing.T) {
	...
	if _, err := NewProfileService(repo, &errorStateRepo{loadErr: errors.New("state load failed")}, nil, nil, nil).AgentSet(domainprofile.AgentCodex, "relay-a"); err == nil || !strings.Contains(err.Error(), "state load failed") {
	...
	codexBase.snapshotErr = errors.New("snapshot failed")
	codex := &fakeCodexSyncer{codexBase}
	if _, err := NewProfileService(repo, &fakeStateRepo{}, codex, nil, nil).AgentSet(domainprofile.AgentCodex, "relay-a"); err == nil || !strings.Contains(err.Error(), "snapshot failed") {
		t.Fatalf("AgentSet(CaptureAgents failure) err=%v, want snapshot failed", err)
	}

	if _, err := NewProfileService(repo, &errorStateRepo{loadErr: errors.New("state load failed")}, codex, nil, nil).Clear(domainprofile.AgentCodex); err == nil || !strings.Contains(err.Error(), "state load failed") {
...
func TestProfileMutationAdditionalFailureBranches(t *testing.T) {
	...
	svc := NewProfileService(repo, &errorStateRepo{loadErr: errors.New("state load failed")}, nil, nil, nil)
	if _, err := svc.Add("relay-b", AddProfileInput{BaseURL: "https://relay.example/v1", APIKey: "sk-b"}); err == nil || !strings.Contains(err.Error(), "state load failed") {
...
func TestStateAndBindingsAdditionalBranches(t *testing.T) {
	...
	svc := NewProfileService(repo, stateRepo, codex, nil, nil)
	...
	if stateRepo.state.CodexProfiles["relay-a"].ConfigPath == "" {
		t.Fatalf("CodexProfiles after syncProfileAfterMutation = %+v, want config path", stateRepo.state.CodexProfiles["relay-a"])
	}
	...
	if stateRepo.state.CodexProfiles["relay-a"].ConfigPath == "" {
		t.Fatalf("CodexProfiles after syncCodexProfile = %+v, want relay-a config path updated", stateRepo.state.CodexProfiles["relay-a"])
	}
	...
	if err := NewProfileService(repo, &errorStateRepo{loadErr: errors.New("load failed")}, nil, nil, nil).clearCodexSourceProfileIfMatches("relay-a"); err == nil || !strings.Contains(err.Error(), "load failed") {
		t.Fatalf("clearCodexSourceProfileIfMatches(load failure) err=%v, want load failed", err)
	}

	codex = &fakeCodexSyncer{newFakeAgentSyncer("/tmp/codex/config.toml")}
	codex.statusErr = errors.New("status failed")
	if _, err := NewProfileService(repo, &fakeStateRepo{}, codex, nil, nil).State(); err == nil || !strings.Contains(err.Error(), "status failed") {
		t.Fatalf("State(status failure) err=%v, want status failed", err)
	}

	if _, err := NewProfileService(repo, &errorStateRepo{loadErr: errors.New("load failed")}, nil, nil, nil).boundAgentsForProfile("relay-a"); err == nil || !strings.Contains(err.Error(), "load failed") {
...
func TestRestoreAndDoctorAdditionalBranches(t *testing.T) {
	...
	codex := &fakeCodexSyncer{newFakeAgentSyncer("/tmp/codex/config.toml")}
	if _, err := NewProfileService(repo, seq, codex, nil, nil).Restore(domainprofile.AgentCodex, "backup-1"); err == nil || !strings.Contains(err.Error(), "save failed") {
	...
	if _, err := NewProfileService(repo, seq, nil, nil, nil).Restore(domainprofile.AgentClaude, "backup-1"); err == nil || !strings.Contains(err.Error(), "invalid agent") {
		t.Fatalf("Restore(missing claude syncer) err=%v, want invalid agent", err)
	}

	doctorSvc := NewProfileService(&errorProfileRepo{listErr: errors.New("list failed")}, &fakeStateRepo{}, nil, nil, nil)
	...
	doctorSvc = NewProfileService(repo, &fakeStateRepo{}, &fakeCodexSyncer{newFakeAgentSyncer("/tmp/codex/config.toml")}, nil, nil)
...
func TestBindingHelpersAdditionalErrorBranches(t *testing.T) {
	...
	if _, err := NewProfileService(repo, &fakeStateRepo{}, nil, nil, nil).agentSetLocked(domainprofile.Agent("bad"), "relay-a"); err == nil {
		t.Fatal("agentSetLocked(invalid agent) unexpectedly succeeded")
	}

	if _, err := NewProfileService(repo, &errorStateRepo{loadErr: errors.New("load failed")}, nil, nil, nil).agentSetLocked(domainprofile.AgentCodex, "relay-a"); err == nil || !strings.Contains(err.Error(), "load failed") {
	...
	claude := &fakeClaudeSyncer{claudeBase}
	if _, _, _, err := NewProfileService(repo, &fakeStateRepo{}, nil, claude, nil).syncProfileToAgent(domainprofile.AgentClaude, repo.profiles["relay-a"], "backup-1", domainprofile.State{}, false); err == nil || !strings.Contains(err.Error(), "sync failed") {
		t.Fatalf("syncProfileToAgent(claude sync failure) err=%v, want sync failed", err)
	}

	if _, err := NewProfileService(repo, &fakeStateRepo{}, nil, nil, nil).snapshotCurrentConfig(domainprofile.AgentClaude, "relay-a", "backup-1", time.Now().UTC()); err == nil || !strings.Contains(err.Error(), "invalid agent") {
	...
	if err := NewProfileService(repo, state, nil, nil, nil).syncProfileToBoundAgents(repo.profiles["relay-a"], &state.state, time.Now().UTC()); err == nil || !strings.Contains(err.Error(), "invalid agent") {
