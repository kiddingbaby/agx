...
func TestStatusClearDefaultProfileRemoveProfileAndDeleteBackup(t *testing.T) {
	...
	if strings.Contains(string(content), "profile = \"agx/relay-a\"") {
	...
	if strings.Contains(string(content), "[profiles.\"agx/relay-a\"]") {
...
func TestStatusTracksManagedAndUnmanagedConfiguredProfiles(t *testing.T) {
	dir := t.TempDir()
	configPath := filepath.Join(dir, ".codex", "config.toml")
	...
	content := strings.Join([]string{
		"profile = \"manual\"",
		"",
		"[model_providers.manual_provider]",
		"name = \"manual relay\"",
		"base_url = \"https://manual.sub2api.example/v1\"",
		"wire_api = \"responses\"",
		"env_key = \"MANUAL_API_KEY\"",
		"",
		"[profiles.manual]",
		"model_provider = \"manual_provider\"",
		"model = \"gpt-4.1\"",
	...
	status, err := NewSyncer(configPath, filepath.Join(dir, "backups"), "agx").Status()
	if err != nil {
		t.Fatalf("Status() error = %v", err)
	}
	if status.DefaultProfileName != "" {
		t.Fatalf("DefaultProfileName = %q, want unmanaged manual ignored", status.DefaultProfileName)
	}
	if status.ActiveProfileName != "manual" {
		t.Fatalf("ActiveProfileName = %q, want manual", status.ActiveProfileName)
	}
	if _, ok := status.ManagedProfilesByID["manual"]; ok {
		t.Fatalf("ManagedProfilesByID = %+v, want manual excluded from AGX managed profiles", status.ManagedProfilesByID)
	}
	if got := status.ManagedProfilesByID["relay-a"]; got.BaseURL != "https://relay-a.example/v1" {
		t.Fatalf("ManagedProfilesByID[relay-a] = %+v, want managed profile included", got)
	}
	if got := status.UnmanagedProfilesByID["manual"]; got.BaseURL != "https://manual.sub2api.example/v1" || got.ProviderName != "manual relay" || got.EnvKey != "MANUAL_API_KEY" || got.RelayType != "sub2api" || got.Model != "gpt-4.1" || got.ReviewModel != "gpt-4.1-mini" {
		t.Fatalf("UnmanagedProfilesByID[manual] = %+v, want discovered unmanaged profile metadata", got)
	}
}

func TestExtractUnmanagedProfilesInfersRelayTypes(t *testing.T) {
	content := strings.Join([]string{
		"[model_providers.newapi_main]",
		"name = \"newapi-main\"",
		"base_url = \"https://newapi.example/v1\"",
		"wire_api = \"responses\"",
		"",
		"[profiles.newapi]",
		"model_provider = \"newapi_main\"",
		"",
		"[model_providers.sub2api_main]",
		"name = \"sub2api-main\"",
		"base_url = \"https://pincc.example/v1\"",
		"wire_api = \"responses\"",
		"",
		"[profiles.sub2api]",
		"model_provider = \"sub2api_main\"",
		"",
		"[model_providers.custom_main]",
		"name = \"my relay\"",
		"base_url = \"https://relay.example/v1\"",
		"wire_api = \"responses\"",
		"",
		"[profiles.custom]",
		"model_provider = \"custom_main\"",
	}, "\n")

	profiles := extractUnmanagedProfiles(content)
	if got := profiles["newapi"].RelayType; got != "newapi" {
		t.Fatalf("profiles[newapi].RelayType = %q, want newapi", got)
	}
	if got := profiles["sub2api"].RelayType; got != "sub2api" {
		t.Fatalf("profiles[sub2api].RelayType = %q, want sub2api", got)
	}
	if got := profiles["custom"].RelayType; got != "custom" {
		t.Fatalf("profiles[custom].RelayType = %q, want custom", got)
	}
}
...
func TestCodexConfigHelperBranchesForUnmanagedAndEmptyConfigs(t *testing.T) {
	...
	if _, err := stripManagedBlock(beginMarker+"\n# orphan", ""); err == nil {
		t.Fatal("stripManagedBlock(orphan) unexpectedly succeeded")
	}
}

func TestCodexConfigRejectsIncompleteManagedBlock(t *testing.T) {
	dir := t.TempDir()
	configPath := filepath.Join(dir, ".codex", "config.toml")
	...
	if err := os.WriteFile(configPath, []byte("profile = \"old\"\n\n"+beginMarker+"\n# orphan\n"), 0600); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
	...
	if _, err := syncer.Status(); err == nil {
		t.Fatal("Status() unexpectedly succeeded with incomplete managed block")
	}
	if _, err := syncer.ClearDefaultProfile(); err == nil {
		t.Fatal("ClearDefaultProfile() unexpectedly succeeded with incomplete managed block")
	}
	if _, err := syncer.RemoveProfile("relay-a"); err == nil {
		t.Fatal("RemoveProfile() unexpectedly succeeded with incomplete managed block")
	}
}
...
func TestCodexConfigAdditionalSuccessAndErrorBranches(t *testing.T) {
	...
	text := string(data)
	if !strings.Contains(text, "profile = \"agx/relay-a\"") || strings.Contains(text, "[profiles.\"agx/relay-b\"]") {
		t.Fatalf("config=%q want agx/relay-a preserved as root and relay-b removed", text)
	}
	...
	if !strings.Contains(string(data), "[profiles.\"agx/relay-a\"]") {
...
func TestCodexConfigHelperAndErrorBranches(t *testing.T) {
	...
	section, name = parseManagedSection(`[profiles."relay-a".features]`)
	if section != "profile_nested" || name != "relay-a" {
		t.Fatalf("parseManagedSection(nested profile section) = (%q,%q), want profile_nested relay-a", section, name)
	}
	...
	block = strings.Join([]string{
		...
		`[profiles."agx/relay-a"]`,
		`model_provider = "agx/relay-a"`,
		`[profiles."agx/relay-a".features]`,
		`sandbox = "workspace-write"`,
		`[model_providers."agx/\"relay-a\".bad"]`,
		`name = "\"relay-a\".bad"`,
		endMarker,
	}, "\n")
	profiles = extractManagedProfiles(block)
	if _, ok := profiles[`"relay-a".bad`]; ok {
		t.Fatalf("extractManagedProfiles() unexpectedly kept invalid quoted profile name: %+v", profiles)
	}
	if got := profiles["relay-a"].Extras; len(got) != 2 || got[0] != `[profiles."agx/relay-a".features]` || got[1] != `sandbox = "workspace-write"` {
		t.Fatalf("extractManagedProfiles() nested extras = %v, want nested table preserved under relay-a", got)
	}
}
...
func TestCodexConfigAdditionalBranchCoverage(t *testing.T) {
	...
	if content, exists, err := fileutil.ReadIfExists(filepath.Join(blocker, "config.toml")); err == nil || exists || content != "" {
		t.Fatalf("fileutil.ReadIfExists(file parent) = (%q,%v,%v), want error", content, exists, err)
	}
	...
	if got := codexProfileName("relay-a"); got != "agx/relay-a" {
		t.Fatalf("codexProfileName() = %q, want agx/relay-a", got)
	}
