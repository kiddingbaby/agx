...
func TestStateRepositoryAcquireLockAndLoadEmpty(t *testing.T) {
	...
	if state.Codex.SourceProfile != "" || state.Claude.SourceProfile != "" || state.Gemini.SourceProfile != "" || len(state.CodexProfiles) != 0 {
...
func TestRepositoryLoadOneAndSaveOneAdditionalBranches(t *testing.T) {
	...
	if _, err := repo.Get("relay-b"); err == nil || !strings.Contains(err.Error(), "api key is required") {
		t.Fatalf("Get(invalid api key file) err=%v, want api key validation error", err)
	}
