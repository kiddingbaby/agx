...
func (s *Syncer) Sync(profile domainprofile.Profile) (*ports.ClaudeSyncResult, error) {
	...
	if err := writeAtomicJSON(s.settingsPath, next, 0600); err != nil {
		return nil, err
	}
...
func (s *Syncer) Restore(backupPath string) (string, error) {
	...
	if err := writeAtomicBytes(s.settingsPath, data, 0600); err != nil {
		return "", err
	}
...
func (s *Syncer) CreateBackup(id string, content []byte) (string, error) {
	...
	if err := writeAtomicBytes(path, content, 0600); err != nil {
		return "", err
	}
	return path, nil
}
...
func (s *Syncer) RemoveConfig() (string, error) {
	...
	if err := writeAtomicJSON(s.settingsPath, settings, 0600); err != nil {
		return "", err
	}
	return s.settingsPath, nil
}
...
func writeAtomicJSON(path string, value map[string]any, mode os.FileMode) error {
